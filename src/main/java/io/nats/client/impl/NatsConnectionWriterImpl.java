...
class NatsConnectionWriterImpl implements Runnable, NatsConnectionWriter {
    ...
    final Duration waitForMessage = Duration.ofMillis(100); // This can be long since no one is sending
    final Duration reconnectWait = Duration.ofMillis(1); // This should be short, since we are trying to get the reconnect through

    final int maxAccumulate = Options.MAX_MESSAGES_IN_NETWORK_BUFFER;
    ...
    public int doWriteMessages() throws IOException, InterruptedException, BufferOverflowException {
        int messagesSent =0;
        NatsStatistics stats = this.connection.getNatsStatistics();
        NatsMessage msg = getNextNatsMessage();
        while (msg != null) {
            sendMessageBatch(msg, dataPort, stats);
            msg = getNextNatsMessage();
            messagesSent++;
        }
        return messagesSent;
    }

    private NatsMessage getNextNatsMessage() throws InterruptedException {
        NatsMessage msg;
